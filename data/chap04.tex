...
要实现用离子实现量子门，尤其是用脉冲激光实现离子量子比特门，首先需要选择合适的离子体系，并且需要对选定离子的能级结构和量子态控制原理和方法有着充分的了解。出于历史和现实的原因，我们实验室选择$Yb^+$离子作为实现量子计算的物理平台。在接下的几节中，我将阐述用镱离子实现量子计算的一些重要的概念，如镱离子的能级结构和比特编码方式、镱离子的态初始化、镱离子的探测、镱离子的操控等。
